...
def generate_pitch_control_for_event(event_id, events, tracking_home, tracking_away, params, field_dimen = (106.,68.,), n_grid_cells_x = 50):
    ...
    pass_team = events.loc[np.searchsorted(events["End Frame"], event_id)].Team
    ball_start_pos = np.array([tracking_away.loc[event_id]['ball_x'],tracking_away.loc[event_id]['ball_y']])
